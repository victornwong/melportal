...
class doculinks_lb_onSelect implements org.zkoss.zk.ui.event.EventListener
{
	public void onEvent(Event event) throws UiException
	{
		...
		fileowner = lbhand.getListcellItemLabel(selitem,4);
...
// 14/01/2015: check selected file's owner, if same as login-id, can delete
void deleteFile_2(Div idiv, String iprefix, String idocudx)
{
	if(selected_file_id.equals("")) return;

	if(useraccessobj.accesslevel < 9)
	{
		if( !fileowner.equals( useraccessobj.username ) )
		{
			guihand.showMessageBox("Sorry, you're not the owner of this file, cannot delete!");
			return;
		}
	}
...
void saveFileToDMS_2(String ilnkc, String ifilename, String ifullpath, String ifiletype, String ifileext, String ifiledesc)
{
	Sql ds_sql = sqlhand.DMS_Sql();
	if(ds_sql == null) return false;
